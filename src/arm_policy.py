...
class ArmPolicy:
    ...
    def get_q_value_map(self, state):
        ...
        for dimension in range(0, len(self.goals)): # go through all the dimensions (3, in this case)
            ...
            if curr_state[dimension] < self.goals[dimension] - 0.03: # if we haven't reached the goal yet
            ...
            elif curr_state[dimension] > self.goals[dimension] + 0.03: # if we've gone past the goal
    ...
    def get_action(self, state):
        # return best of actions, q-val distribution, and probability distribution
        # q_values = self.get_q_values(state)
        action = np.argmax(self.get_q_value_map(state))
        return action
